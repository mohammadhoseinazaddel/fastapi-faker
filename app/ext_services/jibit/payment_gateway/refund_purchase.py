...
def jibit_pay_gw_refund_purchase(
        client_ref_num: str,
        purchase_id: int,
        amount: int = 0,
    ...
    try:
        response = requests.post(settings.JIBIT_PAY_GW_BASE_URL + f'/v3/purchases/refund', headers=headers,
                                 json=json_data)
    ...
    res = {
        ...
        "batch_id": response['batchId'] if 'batchId' in response else None,
        "transfer_id": response['transferId'] if 'transferId' in response else None,
    }
    return res
